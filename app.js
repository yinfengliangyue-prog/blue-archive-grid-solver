(function () {
  ...
  function locateBoardFromPixels(image, rows, cols) {
    ...
    const test=(x,y,cell)=>{let centers=0,coloredCenters=0,edges=0,insideEdges=0;const radius=Math.max(2,cell*.18),strip=Math.max(1,cell*.04);for(let r=0;r<rows;r+=1)for(let c=0;c<cols;c+=1){const center=mean(x+(c+.5)*cell,y+(r+.5)*cell,radius);centers+=center;if(center>.36)coloredCenters+=1;}for(let c=0;c<=cols;c+=1)edges+=rectMean(gradX,x+c*cell-strip,y,x+c*cell+strip,y+rows*cell,765);for(let r=0;r<=rows;r+=1)edges+=rectMean(gradY,x,y+r*cell-strip,x+cols*cell,y+r*cell+strip,765);for(let c=0;c<cols;c+=1)insideEdges+=rectMean(gradX,x+(c+.5)*cell-strip,y,x+(c+.5)*cell+strip,y+rows*cell,765);for(let r=0;r<rows;r+=1)insideEdges+=rectMean(gradY,x,y+(r+.5)*cell-strip,x+cols*cell,y+(r+.5)*cell+strip,765);const periodic=Math.max(0,edges/(cols+rows+2)-insideEdges/(cols+rows));const rightBias=(x+cols*cell*.5)/w;return centers/(rows*cols)*4.2+coloredCenters/(rows*cols)*2.1+periodic*1.8+edges/(cols+rows+2)*.35+rightBias*.22;};
